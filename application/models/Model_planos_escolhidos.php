...
Class Model_planos_escolhidos extends CI_Model {

    //insert
    public function vincularPlanos($dados = null) {
        if ($dados != NULL):
  ...
  public function alteraStatusBeneficiario($dados = null, $id = null) {
        if (($dados && $id) != null):
            $this->db->update('tb_beneficiario', $dados, array('id_beneficiario' => $id));
        endif;
    }
    public function updatePlanosEscolhidos($dados = null, $id = null) {
        if (($dados && $id) != null):
            $this->db->update('tb_planos_escolhidos', $dados, array('id_plano_esc' => $id));
        endif;
    }

    public function DeletarVinculo($id = null) {
        if (($id) != null):
            $this->db->delete('tb_planos_escolhidos', array('id_plano_esc' => $id));
        endif;
    }
